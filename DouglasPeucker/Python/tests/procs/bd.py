...
def insert_non_simplified_points(conn, points, execution_count):
    cur = conn.cursor()
    
    # Generate non_simplified_points table name based on execution count and current datetime
    non_simplified_table_name = f"non_simplified_table_{execution_count}_{datetime.now().strftime('%d_%m_%y_%H_%M_%S')}"

    # Create the non_simplified_points table with the dynamically generated name
    create_non_simplified_table_sql = f"""
        CREATE TABLE {non_simplified_table_name} (
    ...
    # Insert points into the dynamically created non_simplified_points table
    for point in points:
        x, y = point
        sql = f"INSERT INTO {non_simplified_table_name} (nome, ponto) VALUES (%s, ST_SetSRID(ST_MakePoint(%s, %s), 4326))"
...
def insert_simplified_points(conn, simplified_points, non_simplified_id):
    cur = conn.cursor()
    
    # Generate simplified_points table name based on non_simplified_id and current datetime
    simplified_table_name = f"simplified_table_{non_simplified_id}_{datetime.now().strftime('%d_%m_%y_%H_%M_%S')}"

    # Create the simplified_points table with the dynamically generated name
    create_simplified_table_sql = f"""
        CREATE TABLE {simplified_table_name} (
            id SERIAL PRIMARY KEY,
    ...
    # Insert simplified points into the dynamically created simplified_points table
    for point in simplified_points:
        x, y = point
        sql = f"INSERT INTO {simplified_table_name} (simplified_nome, simplified_ponto, non_simplified_id) VALUES (%s, ST_SetSRID(ST_MakePoint(%s, %s), 4326), %s)"
